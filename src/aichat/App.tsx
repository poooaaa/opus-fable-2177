...
export default function App() {
  ...
  const [activeTooltip, setActiveTooltip] = useState<{ id: string; type: 'liked' | 'disliked' | 'refreshed' | 'copied' } | null>(null);
  ...
  const [copiedSuggestion, setCopiedSuggestion] = useState<{ id: string; rect: DOMRect } | null>(null);
      ...
      try {
        ...
        if (res.ok) {
          ...
          const list = rawList.filter(
            (item: string) => typeof item === 'string' && item.toLowerCase() !== query.toLowerCase()
          );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div id="conversation-stream" className="flex-1 flex flex-col justify-start w-full overflow-y-auto max-h-[calc(100vh-140px)] pr-0.5 no-scrollbar">
          {messages.map((msg) => {
            if (msg.role === 'user') {
              return (
            ...
            return (
              <div key={msg.id} id={`assistant-message-${msg.id}`} className="w-full mt-7 sm:mt-9">
                ...
                >
                  {/* Left Action Buttons */}
                  <div id="action-buttons-group" className="flex items-center gap-4 sm:gap-4.5 text-[#73737c]">
